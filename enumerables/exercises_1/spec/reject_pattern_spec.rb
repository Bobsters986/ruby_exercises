RSpec.describe 'reject pattern' do
  ...
  it 'removes floats' do
    ...
    not_numbers = []
    elements.each do |element|
      not_numbers << element if element.is_a?(String) || element.is_a?(Integer)
    end
  ...
  it 'removes animals starting with vowels' do
    ...
    remaining = []
    animals.each do |animal|
      remaining << animal unless animal.start_with?("a", "e", "i", "o", "u", "y")
    end
  ...
  it 'removes upcased words' do
    ...
    remaining = []
    words.each do |word|
      remaining << word if word != word.upcase
    end
